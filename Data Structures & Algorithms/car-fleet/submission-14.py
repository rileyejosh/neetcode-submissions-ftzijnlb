class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        for car in cars:
            ...
            # if the time that it takes for the current car to reach the destination
            # is less than or equal to the time of the current fleet
            # then this car belongs to the current fleet
            # so move on to the next car
            if fleets and time <= fleets[-1]:
                continue
            # the car arrives later than the current fleet
            # so form new fleet which becomes the new current fleet
            fleets.append(time)
        return len(fleets)
