class Solution {
    ...
    isValidSudoku(board) {
        ...
        */
        for(let i = 0; i < ROW; i++) {
            for(let j = 0; j < COL; j++) {
                if(boardSet.has(JSON.stringify(board[i][j]))) 
                    return false;
                else {
                       if(board[i][j] !== ".")
                            boardSet.add(JSON.stringify(board[i][j]));
                }
            }
            boardSet.clear();
        }
        ...
        for(let i = 0; i < ROW; i++) {
            for(let j = 0; j < COL; j++) {
                if(boardSet.has(JSON.stringify(board[j][i]))) 
                    return false;
                else {
                            if(board[j][i] !== ".")
                                boardSet.add(JSON.stringify(board[j][i]));
                }
        ...
        for(let i = 0; i < ROW; i+=3) {
            for(let j = 0; j < COL; j+=3) {
                for(let n = i; n < i+3; n++ ) {
                    for(let m = j; m < j+3; m++) {
                        if(boardSet.has(JSON.stringify(board[n][m]))) 
                            return false;
                        else {
                            if(board[n][m] !== ".")
                                boardSet.add(JSON.stringify(board[n][m]));
                        }
                 }
                }
                boardSet.clear();
